...
namespace TextFx
{
	...
	public class TextFxAnimationManager
	{
		...
		string m_textFxRootDirectoryPath = "";
		...
		void Init(MonoBehaviour monoInstance)
		{
			...
			// Assumes that the TextFx script is located in 'TextFx/Scripts'
			MonoScript monoScript = MonoScript.FromMonoBehaviour (monoInstance);
			string managerScriptPath = AssetDatabase.GetAssetPath ( monoScript);
			m_textFxRootDirectoryPath = managerScriptPath.Replace ("Scripts/" + monoScript.name + ".cs", "");
#endif
		}
		...
		string GetPresetAnimationSectionFolderName(PRESET_ANIMATION_SECTION section)
		{
			switch (section)
			{
				case PRESET_ANIMATION_SECTION.INTRO:
					return ANIM_INTROS_FOLDER_NAME;
				case PRESET_ANIMATION_SECTION.MAIN:
					return ANIM_MAINS_FOLDER_NAME;
				case PRESET_ANIMATION_SECTION.OUTRO:
					return ANIM_OUTROS_FOLDER_NAME;
				default:
					return "";
			}
		}
		...
		{
			// Can't be used in builds yet, since it relies of loading raw unreferenced json text configuration files
			...
			if(animationName != "None")
			{
				...
				string path = m_textFxRootDirectoryPath + "AnimationConfigs/" + GetPresetAnimationSectionFolderName(section) + "/" + animationName + ".txt";
				path = path.Replace("\n", "");
				path = path.Replace("\r", "");
				...
				if(animation_config_data != null)
				{
					...
					ImportData(animation_config_data.text, preset_anim_section, section, true);
		...
		public void UpdateText(string text_string, GuiTextDataHandler textData, bool white_space_meshes)
		{
			...
			m_num_meshes = textData.NumVerts / 4;
...
//			Debug.Log ("PrepareAnimationData () " + what_to_update);
		...
		{
			float delta_time = m_time_type == AnimationTime.GAME_TIME && Application.isPlaying ? Time.time - m_last_time : Time.realtimeSinceStartup - m_last_time;
...
//				lerpSyncDuration = 0f;
		...
		{
			if(forcePopulate || m_current_mesh_verts == null || m_current_mesh_verts.Length != m_num_meshes * 4)
			{
				...
				for (int letter_idx = 0; letter_idx < m_letters.Length; letter_idx++)
				{
					if(!m_letters[letter_idx].StubInstance)
					{
						...
						vertColours = m_letters[letter_idx].BaseColour;
						...
						m_current_mesh_colours[mesh_idx * 4] = vertColours.top_left;
						m_current_mesh_colours[mesh_idx * 4 + 1] = vertColours.top_right;
						m_current_mesh_colours[mesh_idx * 4 + 2] = vertColours.bottom_right;
						m_current_mesh_colours[mesh_idx * 4 + 3] = vertColours.bottom_left;
	...
	//					m_colourVertMapping = new int[]{1,0,3,2};
		...
		public string ExportDataAsPresetSection(bool saveSampleTextInfo = true)
		{
			...
			Boomlagoon.JSON.tfxJSONObject json_data = new Boomlagoon.JSON.tfxJSONObject();
			...
			Boomlagoon.JSON.tfxJSONArray effect_settings_options = new Boomlagoon.JSON.tfxJSONArray();
		...
		public string ExportData(bool hard_copy = false)
		{
			...
			Boomlagoon.JSON.tfxJSONArray letter_animations_data = new Boomlagoon.JSON.tfxJSONArray();
			if(m_master_animations != null)
			{
		...
		public void ImportPresetAnimationSectionData(string data, bool force_clear_old_audio_particles = false)
		{
			...
			Boomlagoon.JSON.tfxJSONObject json_data = Boomlagoon.JSON.tfxJSONObject.Parse(data, true);
			...
			if(json_data != null)
			{
				...
				if(json_data.ContainsKey("PRESET_EFFECT_SETTINGS"))
				{
					...
					foreach(Boomlagoon.JSON.tfxJSONValue effectSettingData in json_data["PRESET_EFFECT_SETTINGS"].Array)
					{
...
//				m_preset_effect_settings = new List<PresetEffectSetting>();
		...
		public void ImportData(string data, bool force_clear_old_audio_particles = false)
		{
			...
			if(json_data != null)
			{
				...
				foreach(Boomlagoon.JSON.tfxJSONValue animation_data in json_data["LETTER_ANIMATIONS_DATA"].Array)
				{
