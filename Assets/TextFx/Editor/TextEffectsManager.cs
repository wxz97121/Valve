...
#if UNITY_4 || UNITY_4_6 || UNITY_5_0
			window.title = "TextFx Editor";
#else
			window.titleContent = new GUIContent( "TextFx Editor",  AssetDatabase.LoadAssetAtPath(Instance.m_textFxRootDirectoryPath + "Editor/Icons/EditorTitleIcon.png", typeof(Texture2D)) as Texture2D);
#endif
		}
...
//			m_save_button_texture = AssetDatabase.LoadAssetAtPath(m_textFxRootDirectoryPath + "Editor/Icons/SaveButton.png", typeof(Texture2D)) as Texture2D;
		...
		void LoadInPresetAnimationNames()
		{
			...
			TextAsset config_titles_file = AssetDatabase.LoadAssetAtPath(m_textFxRootDirectoryPath + "AnimationConfigs/AnimationConfigs2.txt", typeof(TextAsset)) as TextAsset;
		...
		void LoadInAnimationPartNames(string fileName, ref string[] configList)
		{
			...
			// Load in the names and categories of all the animation configurations available from the anim config file
			TextAsset config_title_file = AssetDatabase.LoadAssetAtPath(m_textFxRootDirectoryPath + "AnimationConfigs/" + fileName + ".txt", typeof(TextAsset)) as TextAsset;
		...
		void DrawGUIAnimationSettings(TextFxAnimationManager.PRESET_ANIMATION_SECTION section, string folder_name, ref float gui_y_offset, ref bool foldout, ref int selected_index, ref TextFxAnimationManager.PresetAnimationSection preset_anim_section, string[] anim_titles)
		{
			...
			if(foldout && selected_index > 0)
			{
				...
				if(!gui_changed && GUI.changed)
				{
					...
					exitPauseAction.m_duration_progression.SetConstant(preset_anim_section.m_exit_pause ? preset_anim_section.m_exit_pause_duration : TextFxAnimationManager.INACTIVE_EXIT_PAUSE_DURATION);

					m_animation_manager.PrepareAnimationData(ANIMATION_DATA_TYPE.DURATION);

					m_animation_manager.UpdatePresetAnimSectionActionIndexes();
				}
